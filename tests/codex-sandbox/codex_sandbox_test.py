...
AGENT_SANDBOX_DOCKERFILE = ROOT / "libexec" / "agent-sandbox" / "Dockerfile"
AGENT_WRAPPERS_PROFILE = ROOT / "libexec" / "agent-sandbox" / "agent-wrappers-path.sh"
DOTFILES_PROFILE = ROOT / "libexec" / "agent-sandbox" / "dotfiles-profile.sh"
SANDBOX_GITCONFIG = ROOT / "libexec" / "agent-sandbox" / "gitconfig"
...
class AgentSandboxImageTest(unittest.TestCase):
    def test_login_profile_restores_agent_wrappers_path(self) -> None:
        ...
        self.assertIn(
            "COPY ./libexec/agent-sandbox/agent-wrappers-path.sh /etc/profile.d/agent-wrappers-path.sh",
    ...
    def test_dotfiles_profile_is_noninteractive_and_image_managed(self) -> None:
        ...
        for source in (
            "./libexec/agent-sandbox/dotfiles-profile.sh",
            "./libexec/agent-sandbox/gitconfig",
            "./config/editorconfig",
            "./config/inputrc",
        ):
