...
6,200-word ranked frontier is rebalanced to a 6,151-word exact minimized acyclic
...
20,403 bytes and cannot produce membership false positives.

Grouped morphology stores 67 exact spelling recipes and 1,698 licensed roots in
...
outlines. The 1,310 output words are lexically front-coded in 384-word blocks,
...
conventional dictionary and rule outlines cover 93.8978% after Zipf weighting
at the exact 40,960-byte budget. This is an in-sample optimization metric: the
cutoff is arbitrary and the model was tuned against the same list. The pinned
input is generated by `generate_wordfreq.py` from `wordfreq==3.1.1`; its SHA-256
is `831507abd1bf89dce3d60bd19a23629eeb7fc5f926a4d108ba1e8448fafcb4a4`.
It is not a 20,000-token text
...
and RFC 9110 measured 87.22% conventional token coverage after apostrophe
normalization, ranging from 82.49% to 91.19% by corpus. The same frozen model
scores 99.39%, 97.05%, 93.90%, and 91.50% when the frequency-list denominator is
extended to its first 5,000,
